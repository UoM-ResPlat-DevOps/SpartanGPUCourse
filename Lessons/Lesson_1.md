...
* The compiler will run in paralle if it can, and will attempt to resolve any data dependencies, and data movement. If successful, it produces a kernel to run on the accelerator or it let's the programmer know that the region was not available for parallel execution.
...
* The `kernels` directive, used in this course, is a general case statement. They direct the compiler to produce and arbitrary numebr of kernels of arbitrary dimensions, to execute in sequence, and to offload code sections to the accelerator. 
* The `parallel` directive allows further finer-grained control of how the compiler will attempt to structure work on the accelerator, by specifying specific dimensions of parallelisation (e.g., `vector` and `gang` parallelism). 
-- *Slide End* --
...
* With NVCC the `-arch` flag specifies the name of the NVidia GPU architecture that the CUDA files will be compiled for and `-gencode` allows for more PTX (Parallel Thread Execution) generations, and can be repeated many times for different architectures.
* Architectures can be specified with `sm_XX` and `compute_XX`, for real and virtual architectures respectively.
* Compile for the architecture (both virtual and real), that represents the GPUs you wish to target. i.e., `-gencode arch=compute_XX,code=sm_XX` and repeat the sequence for each target.
-- *Slide End* --
...
* The execution configuration allows programmers specification how many groups of threads (thread blocks, or blocks) and how many threads they would like each thread block to contain. The general syntax for this is:
...
* `someKernel<<<10, 10>>()` Ten blocks, ten threads per block, will run a hundred times. 
-- *Slide End* --
...
* The number of threads in a block is limited to 1024. Beyond this, coordination is required across multiple thread blocks. This is achieved through the variable `blockDim.x`, which gives the number of threads in a block. When used with `blockIdx.x` and `threadIdx.x`, coordination parallelisation is achieved across multiple blocks of multiple threads.
...
* The number of threads in a grid may be smaller than the size of a data set (e.g., a grid of 2500 threads, an array of 10000 datasets). To resolve this, a grid-stride loop can be used within the kernel.
